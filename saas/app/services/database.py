...
class PostgresBackend(DatabaseBackend):
    ...
    @property
    def supports_native_json(self) -> bool:
        # psycopg v3 cannot auto-adapt dicts via %s placeholders.
        # Returning False makes DB classes json.dumps() before insert,
        # which PG JSONB columns accept as text input.  Reads still
        # return Python dicts because psycopg auto-deserialises JSONB.
        return False

    @staticmethod
    def _normalize_row(row: dict) -> dict:
        """Convert datetime values to ISO strings for API consistency."""
