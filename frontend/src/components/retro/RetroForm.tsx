...
interface RetroFormProps {
  onSubmit: (smiles: string, maxDepth: number, beamWidth: number) => void;
  loading?: boolean;
  error?: string | null;
}

export function RetroForm({ onSubmit, loading, error }: RetroFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (smiles.trim()) onSubmit(smiles.trim(), maxDepth, beamWidth);
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex gap-2">
        <input
          ...
          placeholder="Enter target SMILES (e.g., CC(=O)OC1=CC=CC=C1C(=O)O for aspirin)"
