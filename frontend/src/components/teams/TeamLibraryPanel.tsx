...
import type { TeamLibraryMoleculeResponse } from "@/api/types";

function TeamMoleculeRow({
  mol,
  teamId,
  onDeleted,
  onUpdated,
}: {
  mol: TeamLibraryMoleculeResponse;
  teamId: number;
  onDeleted: () => void;
  onUpdated: () => void;
}) {
  const { updateTeamMolecule, deleteTeamMolecule, loading } = useTeams();
  ...
  const handleSave = async () => {
    const tagList = editTags
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    await updateTeamMolecule(teamId, mol.id, {
      name: editName,
      tags: tagList,
      notes: editNotes,
    });
  ...
  const handleDelete = async () => {
    const ok = await deleteTeamMolecule(teamId, mol.id);
    if (ok) onDeleted();
  };
    ...
    <div className="border border-border rounded-[var(--radius-sm)] bg-bg-card">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          {mol.name && (
            <span className="text-sm text-text-secondary">{mol.name}</span>
          )}
          {mol.tags.map((t) => (
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-text-muted">{mol.added_by}</span>
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3 space-y-3">
          ...
          {mol.notes && !editing && (
            <p className="text-xs text-text-muted">{mol.notes}</p>
          )}
        </div>
      )}
    </div>
...
function SaveTeamMoleculeForm({
  ...
  const { saveTeamMolecule, loading, error } = useTeams();
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const tagList = tags
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    const result = await saveTeamMolecule(
      teamId,
      smiles,
      name || undefined,
  ...
  return (
    <Card>
      <h3 className="text-base font-semibold mb-3">Save Molecule</h3>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <Button type="submit" disabled={loading || !smiles}>
          {loading ? "Saving..." : "Save to Team Library"}
        </Button>
      </form>
    </Card>
  );
...
function ImportTeamPanel({
  ...
  const { importTeamMolecules, loading, error } = useTeams();
  ...
  const handleImport = async (e: FormEvent) => {
    ...
    const res = await importTeamMolecules(teamId, list, tag || undefined);
    if (res) {
      setResult(res);
      onImported();
    }
  ...
  return (
    <Card>
      <h3 className="text-base font-semibold mb-3">Bulk Import</h3>
...
export function TeamLibraryPanel({ teamId }: { teamId: number }) {
  const { fetchTeamLibrary, loading, error, library } = useTeams();
  const [tagFilter, setTagFilter] = useState("");
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);

  const reload = () => {
    fetchTeamLibrary(teamId, {
  ...
  const totalPages = library ? Math.ceil(library.total / library.per_page) : 0;

  return (
    <div className="space-y-4">
      <div className="grid md:grid-cols-2 gap-4">
        <SaveTeamMoleculeForm teamId={teamId} onSaved={reload} />
        <ImportTeamPanel teamId={teamId} onImported={reload} />
      </div>
        ...
        {library && (
          <>
            <p className="text-xs text-text-muted mb-3">
              {library.total} molecule{library.total !== 1 ? "s" : ""} found
            </p>
            <div className="space-y-2">
              {library.molecules.map((mol) => (
                <TeamMoleculeRow
                  key={mol.id}
                  mol={mol}
                  teamId={teamId}
